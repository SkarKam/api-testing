...
public class TestSteps {
        ...
        public static Board createBoard(RequestSpecification requestSpecification,String name){
            return TestSteps.sendRequest(requestSpecification,Method.POST,name).getBody().as(Board.class);
        }
        ...
        public static <T> Response sendRequest(RequestSpecification request, Method method, T... nameOrPUTBody) {
            ...
            switch (method) {
                ...
                case POST:
                     return RestAssured.given()
                        .spec(request)
                        .queryParam("idOrganization",Dotenv.load().get("ORG_ID"))
        ...
        public static void checkActualVsExpectedResponses(Object actual, Object expected) {
                Assert.assertEquals(actual, expected, "Object are different.");
        }

        public static void deleteBoard(RequestSpecification requestSpecification, Board board){
            checkResponseIsValid(sendRequest(buildRequest(requestSpecification, board.getId()), Method.DELETE));
        }
